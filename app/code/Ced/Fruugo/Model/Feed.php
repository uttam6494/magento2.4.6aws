...
class Feed extends \Magento\AdminNotification\Model\Feed
{
    ...
    const XML_FEED_URL_PATH     = 'system/fruugo/feed_url';
    const XML_FREQUENCY_PATH    = 'system/fruugo/frequency';
    const XML_LAST_UPDATE_PATH  = 'system/fruugo/last_update';
    ...
    public function checkUpdate()
    {
        ...
        $feedXml = $this->getFeedData($this->_objectManager->get('Ced\Fruugo\Helper\Feed')->getEnvironmentInformation());
        ...
        if ($feedXml && $feedXml->channel && $feedXml->channel->item) {
            ...
            $installedModules=$this->_objectManager->get('Ced\Fruugo\Helper\Feed')->getCedCommerceExtensions();
            foreach ($feedXml->channel->item as $item) {
               
                if(!isset($installedModules[(string)$item->module])) {
                    continue;
                }
                if($this->_objectManager->get('Ced\Fruugo\Helper\Feed')->isAllowedFeedType($item)) {
            ...
            if($feedData) {
                $this->_cacheManager->save(json_encode($feedData,true), 'all_extensions_by_cedcommerce');
            }
