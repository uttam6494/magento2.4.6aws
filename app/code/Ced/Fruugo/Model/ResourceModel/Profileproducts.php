...
class Profileproducts extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function _construct()
    {
        ...
        $this->_init('fruugo_profile_products', 'id');
    ...
    public function deleteFromProfile($productId)
    {
        ...
        $condition = "{$this->getTable('fruugo_profile_products')}.product_id = " . $dbh->quote($productId);
        //. " AND {$this->getTable('fruugo/profileproducts')}.profile_id = " . $dbh->quote($profileId);
        $dbh->delete($this->getTable('fruugo_profile_products'), $condition);
        return $this;
    ...
    {
        if ( $productId > 0 ) {
            $profileTable = $this->getTable('fruugo_profile_products');

            $productProfile = \Magento\Framework\App\ObjectManager::getInstance()->get('Ced\Fruugo\Model\Profileproducts')->loadByField('profile_id',$profileId);
