...
namespace Ced\Fruugo\Controller\Adminhtml\Main;
        ...
        if($postData){
            ...
            if ($response && isset($response['hash']) && isset($response['level'])) {
                ...
                try {
                    ...
                    {
                        $path = \Ced\Fruugo\Block\Extensions::HASH_PATH_PREFIX.strtolower($postData['module_name']).'_hash';
                        $this->_feedHelper->setDefaultStoreConfig($path, $response['hash'], 0);
                        $path = \Ced\Fruugo\Block\Extensions::HASH_PATH_PREFIX.strtolower($postData['module_name']).'_level';
