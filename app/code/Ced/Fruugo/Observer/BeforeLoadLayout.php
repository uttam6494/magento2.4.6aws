...
class BeforeLoadLayout implements ObserverInterface
{
    ...
    public function __construct(
        ...
        $this->_feedHelper = $this->_objectManager->get('Ced\Fruugo\Helper\Feed');
    }
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
       ...
       try {
            ...
            $validateArray=array();
            if(!preg_match('/ced/i',$controllerModule)){ return $this; }
            $helper = $this->_objectManager->create('Ced\Fruugo\Helper\Feed');
            ...
            foreach ($modules as $moduleName=>$releaseVersion)
            {
                ...
                $h = $this->_objectManager->create('Ced\Fruugo\Helper\Data')->getStoreConfig(\Ced\Fruugo\Block\Extensions::HASH_PATH_PREFIX.$m.'_hash');
         
                for($i=1;$i<=(int)$this->_objectManager->create('Ced\Fruugo\Helper\Data')->getStoreConfig(\Ced\Fruugo\Block\Extensions::HASH_PATH_PREFIX.$m.'_level');$i++)
                    {  
                        $h = base64_decode($h);
                    }
              ...
              //  print_r($h);die;
                if(is_array($h) && isset($h['domain']) && isset($h['module_name']) && isset($h['license']) && strtolower($h['module_name']) == $m && $h['license'] == $this->_objectManager->create('Ced\Fruugo\Helper\Data')->getStoreConfig(\Ced\Fruugo\Block\Extensions::HASH_PATH_PREFIX.$m))
                 {}else{

                    if($validateArray && count($validateArray)==0){
            ...
            foreach ($result as $moduleName => $response) {
                if ($response && isset($response['hash']) && isset($response['level'])) {
                    ...
                    try {
                        ...
                        {
                            $path = \Ced\Fruugo\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName).'_hash';
                              $this->_feedHelper->setDefaultStoreConfig($path, $response['hash'], 0);
                            $path = \Ced\Fruugo\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName).'_level';
                             $this->_feedHelper->setDefaultStoreConfig($path, $response['level'], 0);
                            $path = \Ced\Fruugo\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName);
    ...
    private function getLicenseActivateUrl()
    {
        if (is_null($this->_licenseActivateUrl)) {
            $this->_licenseActivateUrl = ($this->_feedHelper->getStoreConfig(\Ced\Fruugo\Block\Extensions::LICENSE_USE_HTTPS_PATH) ? 'https://' : 'http://')
