...
class Cache extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     */
    const CACHE_KEY_IDENTIFIER = 'ced_fruugo_';
    ...
    public function setValue($key, $value, array $tags = array(), $lifeTime = NULL)
    {
        ...
        $preparedTags = array(self::CACHE_KEY_IDENTIFIER.'_main');
        foreach ($tags as $tag) {
            $preparedTags[] = self::CACHE_KEY_IDENTIFIER.'_'.$tag;
        }

        $this->cache->save(json_encode($value,true), $cacheKey, $preparedTags, (int)$lifeTime);
    }
    ...
    {
        $tags = array(self::CACHE_KEY_IDENTIFIER.'_'.$tag);
