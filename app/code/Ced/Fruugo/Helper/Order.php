...
class Order extends \Magento\Framework\App\Helper\AbstractHelper
{
            ...
            foreach ( $response ['o:orders']['_value']['o:order'] as $order ) {
                ...
                if (count ( $order ) > 0 && $order['o:orderStatus'] == 'PENDING') {
                    ...
                    if (count ( $resultdata->getData () ) <= 0) {
                        $ncustomer = $this->_assignCustomer ( $order, $customer, $email, $store );
     ...
     */
    public function _assignCustomer($order, $customer, $email, $store=null) {
        if (!  ( $customer->getId () )) {
            try {
                ...
                $this->rejectOrder($e->getMessage () , $order ,NULL, NULL , NULL );
                return false;
            }
        } else {
            $firstname = $order['o:shippingAddress']['o:firstName'];
                ...
                if ($product) {                    
                    if ($product->getStatus () == '1' || ( $prodDisabled == "1") ) {
                        ...
                        if ((is_bool($stockstatus) && $stockstatus)|| ( $prodOutOfStock == "1" ))  {
                            ...
                            $this->rejectOrder($stockstatus, $order , $item, $lineNumber , $quantity );
                            }
                    }
                    else {
                        $autoReject = true;
                        $item_rejected = $item['o:fruugoProductId'].','.$item['o:fruugoSkuId'].','.$item['o:totalNumberOfItems'];
                        $reject_items_arr[] = $item_rejected;
                        $this->rejectOrder(' SKU is Disabled in your System.', $order, $item , $lineNumber , $quantity );
                         }
                } else {
                    $autoReject = true;
                    $item_rejected = $item['o:fruugoProductId'].','.$item['o:fruugoSkuId'].','.$item['o:totalNumberOfItems'];
                    $reject_items_arr[] = $item_rejected;
                    $this->rejectOrder(' Is not Available In your System.', $order ,$item , $lineNumber , $quantity );
                   }
                $taxArray[$sku] = $tax;
...
//            $this->connection->rollback();
            $this->rejectOrder($e->getMessage(),$order ,NULL , NULL , NULL );
            // ($message, $result , $item = null  , $lineNumber = null , $quantity = null )
            return false;
        }
    ...
    */
    public function rejectOrder($message, $result , $item = null  , $lineNumber = null , $quantity = null ) {
...
//       $data = $this->datahelper->rejectOrder (  $result ['purchaseOrderId'] , $lineNumber , $quantity);
    ...
    /*
      ...
      */
      public function putShipOrder($data_ship, $postData, $order_to_complete = [], $order_cancel = [], $mixed = []) {
        ...
        if (($responsedata) && ($fruugo_reference_id)) {
            try {
                $this->saveFruugoShipData ( $shipQty, $fruugomodel, $cancelleditems, $data_ship, $order, $order_to_complete, $order_cancel,$mixed );
                return  "Success" ;
            } catch ( \Exception $e ) {
                return $e->getMessage ();
            }
     ...
     * @Ship by fruugo save process
     */
    public function saveFruugoShipData($shipQty , $fruugomodel,$cancelleditems , $data_ship,$order , $order_to_complete=null, $order_cancel=null, $mixed=null) {
