    ...
    return Column.extend({
        ...
        startView: function (row) {
            if (this.getFeedErrors(row)) {
                ...
                var previewPopup = $('<div/>',{id : 'fruugopopup'+this.getFeedId(row) });
                var data = $.parseJSON(this.getFeedErrors(row));
                var result = '<table class="data-grid" style="margin-bottom:25px"><tr><th style="padding:15px">Sl. No.</th><th style="padding:15px">SKU</th><th style="padding:15px">Status</th><th style="padding:15px">Errors</th></tr>';
